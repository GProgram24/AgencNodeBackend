...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!isValidPassword) {
      ...
      } else if (user.userType == "editor" || "viewer") {
        const creatorDetails = await Creator.findOne({
          _id: userDetails.parentId,
        });
      ...
      // Fetch accountType from the accountDetails
      ...
      return res.json({
        ...
        user: {
          ...
          onboardingProgress:
            userDetails.onboardingProgress !== undefined &&
            userDetails.onboardingProgress !== null
              ? userDetails.onboardingProgress
              : null,
