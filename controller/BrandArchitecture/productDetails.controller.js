...
export const addProductServiceMeta = async (req, res) => {
  try {
    ...
    if (
      ...
      productServiceMeta
        .findOneAndUpdate({ productServiceId: productId }, productData, options)
        .then((result) => {
        ...
        .catch((err) => {
          console.log(
            "Error at adding/updating product description, database error:",
            err
          );
...
// To handle get requests related to product
export const getProductDetails = async (req, res) => {
  try {
    ...
    if (detailType === "products") {
      // Fetch all products for the given brand and return only name and ID
      const products = await productService
        .find({ brand: brand._id }, "name _id")
        .exec();
    ...
    if (selectCollection) {
      ...
      const productMeta = await selectCollection
        .find({ productServiceId: { $in: productIds } })
        .exec();

      if (detailType === "description") {
        // Map the metadata to products
        const productsWithMeta = products.map((product) => {
          const meta = productMeta.find((meta) =>
