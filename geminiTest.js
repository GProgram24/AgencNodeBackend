...
app.post('/test/longform', (async (req, res) => {
    const reqObj = req.body;
    if (mongoose.isValidObjectId(reqObj.productId) && reqObj.productName && reqObj.targetAudience) {
        const productDetails = await productServiceMetaModel.findOne({ productServiceId: reqObj.productId });
        const productData = await productServiceModel.findById(reqObj.productId).select("name");
        console.log(productDetails);
        // Function
        const aiResponse = await getAIResponse(productData.name, reqObj.targetAudience.name, reqObj.targetAudience.Location, productDetails.usp, productDetails.feature, productDetails.description,reqObj.targetAudience.painPoints,productDetails.attributes)
        console.log(aiResponse);
        res.json({ response: aiResponse });

    }
    else { res.json({ response: "Insufficient data" }) }
    res.json({ response: "testing prompt" });
})
);
